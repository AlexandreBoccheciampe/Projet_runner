...
const editorMenuUpper = document.querySelector("#editorMenuUpper");

const advancedOptionsDisplayButton = document.querySelector(
  "#advancedOptionsDisplayButton"
); //  bouton de display du menu des optiosn avancées
const advancedOptionsContainer = document.querySelector(
  "#advancedOptionsContainer"
); //  conteneur des inputs d'URL personnalisés
const advancedOptionsHideButton = document.querySelector(
  "#advancedOptionsHideButton"
); //  bouton pour cacher le menu des options avancées
const backgroundInput = document.querySelector("#backgroundInput");
const melodyInput = document.querySelector("#melodyInput");
const roadInput = document.querySelector("#roadInput");
const aerialInput = document.querySelector("#aerialInput");
const groundInput = document.querySelector("#groundInput");
const advancedInputList = [
  ...
  aerialInput,
  groundInput,
]; //  liste conetannt tous les input du menu avancé
...
exportButton.addEventListener("click", (e) => {
  if (
    ...
    advancedInputList.forEach((e) => {
      if (e.value != "") {
        selectedMap["assets"][e.dataset.key] = e.value;
      }
    });
...
advancedOptionsDisplayButton.addEventListener("click", (e) => {
  advancedOptionsContainer.style.display = "block";
  advancedOptionsDisplayButton.style.display = "none";
  editorMenuUpper.style.top = "7%";
});

advancedOptionsHideButton.addEventListener("click", (e) => {
  advancedOptionsContainer.style.display = "none";
  advancedOptionsDisplayButton.style.display = "block";
  editorMenuUpper.style.top = "19%";
});
